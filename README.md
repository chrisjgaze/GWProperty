...
The `/admin/` dashboard links to the custom-property manager and the read-only existing feed
browser. The custom-property manager uses Decap CMS only for authenticated add/edit forms. Each
custom property is stored as its own JSON file in `public/custom-properties/`.
